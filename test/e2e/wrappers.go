...
// ### TO BE DELETED
func ForceFail() {
	// generate a code that If evirnoment KIND_EXTRA_ARGS is set to "--plugin", then force fail the test
	if value, ok := os.LookupEnv("KIND_EXTRA_ARGS"); ok {
		if value == "--plugin" {
			gomega.Expect(false).To(gomega.Equal(true), "")
		}
	}
}
