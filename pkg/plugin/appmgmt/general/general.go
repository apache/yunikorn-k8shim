...
func NewGeneralAppManagementService(amProtocol cache.ApplicationManagementProtocol,
	sharedContext *cache.SharedContext) *GenericAppManagementService {
...
func (os *GenericAppManagementService) ServiceInit() error {
	os.sharedContext.AddEventHandler(
		&cache.ResourceHandlerCallbacks{
...
func (os *GenericAppManagementService) validatePod(pod *v1.Pod) error {
	if pod.Spec.SchedulerName == "" || pod.Spec.SchedulerName != os.sharedContext.GetClientSet().Conf.SchedulerName {
		...
		return fmt.Errorf("only pod whose spec has explicitly "+
			...
			os.sharedContext.GetClientSet().Conf.SchedulerName, pod.Name, pod.UID, pod.Spec.SchedulerName)
	}
	...
	for name, value := range pod.Labels {
		if name == common.SparkLabelRole && value == common.SparkLabelRoleDriver {
			app.StartCompletionHandler(cache.CompletionHandler{
				CompleteFn: func() {
					podWatch, err := os.sharedContext.GetClientSet().
