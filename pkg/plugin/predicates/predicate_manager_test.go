...
func TestEventsToRegister(t *testing.T) {
	...
	var queueingHintFn framework.QueueingHintFn = func(logger klog.Logger, pod *v1.Pod, oldObj, newObj interface{}) framework.QueueingHint {
	...
	assert.Equal(t, int(events[0].QueueingHintFn(klog.NewKlogr(), nil, "", "")), -1, "wrong fn (0)")
	...
	assert.Equal(t, int(events[1].QueueingHintFn(klog.NewKlogr(), nil, "", "")), -1, "wrong fn (1)")
}
