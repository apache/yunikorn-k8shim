...
func TestRegisterPods(t *testing.T) {
	...
	pod1Cache, _ := context.schedulerCache.GetPod(string(pod1.UID))
	assert.Assert(t, pod1Cache != nil, "expected to find pod 1 in cache")
	pod2Cache, _ := context.schedulerCache.GetPod(string(pod2.UID))
	assert.Assert(t, pod2Cache != nil, "expected to find pod 2 in cache")
	pod3Cache, _ := context.schedulerCache.GetPod(string(pod3.UID))
	assert.Assert(t, pod3Cache == nil, "not expected to find pod 3 in cache")
	pod4Cache, _ := context.schedulerCache.GetPod(string(pod4.UID))
	assert.Assert(t, pod4Cache != nil, "expected to find pod 4 in cache")
	...
	pod1Cache, _ = context.schedulerCache.GetPod(string(pod1.UID))
	assert.Assert(t, pod1Cache == nil, "not expected to find pod 1 in cache")
	pod2Cache, _ = context.schedulerCache.GetPod(string(pod2.UID))
	assert.Assert(t, pod2Cache != nil, "expected to find pod 2 in cache")
	pod3Cache, _ = context.schedulerCache.GetPod(string(pod3.UID))
	assert.Assert(t, pod3Cache == nil, "not expected to find pod 3 in cache")
	pod4Cache, _ = context.schedulerCache.GetPod(string(pod4.UID))
	assert.Assert(t, pod4Cache == nil, "not expected to find pod 4 in cache")
	pod5Cache, _ := context.schedulerCache.GetPod(string(pod5.UID))
	assert.Assert(t, pod5Cache == nil, "not expected to find pod 5 in cache")
}
