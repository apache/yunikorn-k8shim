...
func NewRetryTaskEvent(appId string, taskId string, reason string) RetryTaskEvent {
	return RetryTaskEvent{
		applicationId: appId,
		taskId:        taskId,
		event:         events.RetryTask,
		reason:       reason,
	}
}

func (rt RetryTaskEvent) GetEvent() events.TaskEventType {
	return rt.event
}

func (rt RetryTaskEvent) GetArgs() []interface{} {
	args := make([]interface{}, 1)
	args[0] = rt.reason
	return args
}

func (rt RetryTaskEvent) GetTaskId() string {
	return rt.taskId
}

func (rt RetryTaskEvent) GetApplicationId() string {
	return rt.applicationId
}