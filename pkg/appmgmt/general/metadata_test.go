...
func TestGetUserFromPod(t *testing.T) {
	testCases := []struct {
		...
		{
			description: "General app has user label",
			pod: &v1.Pod{
				ObjectMeta: apis.ObjectMeta{
					Labels: map[string]string{constants.DefaultUserLabel: "test1"},
				},
			},
			expectedUser: "test1",
		},
		{
			description:  "General app has no user label",
			pod:          &v1.Pod{},
			expectedUser: constants.DefaultUser,
		},
		{
			description: "Spark app. Spark operator AppMgmt service is not initialized.",
			pod: &v1.Pod{
				ObjectMeta: apis.ObjectMeta{
					Labels: map[string]string{
